...
- (void)commonInit {
	...
	//_scrollView.backgroundColor = [TheConstantsManager backgroundColor];
...
-(void)setCurrentPageIndexAnimated:(NSInteger)newPageIndex {
	if (_scrollView.frame.size.width > 0 && fabsf(_scrollView.frame.origin.x - (-_gapBetweenPages/2)) < 1e-6) {
        NSLog(@"scrollview frame size width : %f", _scrollView.frame.size.width);
