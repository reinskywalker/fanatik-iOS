...
@synthesize tapDelegate, tagInputField_;
...
- (void)commonInit {
    ...
    tagInputField_.font = [UIFont fontWithName:InterfaceStr(@"default_font_regular") size:14.0];
    tagInputField_.placeholder = @"tag";
    tagInputField_.autocorrectionType = UITextAutocorrectionTypeNo;
    [tagInputField_ addTarget:self action:@selector(textFieldChanged:) forControlEvents:UIControlEventEditingChanged];
...
- (void)reloadTagSubviews {
    ...
    for (NSString *tag in _tags) {
        float width = [tag boundingRectWithSize:CGSizeMake(3000,tagInputField_.frame.size.height)
                                     ...
                                     attributes:@{NSFontAttributeName:[UIFont fontWithName:InterfaceStr(@"default_font_bold") size:14.0]}
        ...
        tagLabel.font = [UIFont fontWithName:InterfaceStr(@"default_font_bold") size:14.0];
        ...
        if (_mode == TLTagsControlModeEdit) {
            ...
            buttonFrame.origin.x = labelFrame.size.width - 10;
            
            deleteTagButton.frame = buttonFrame;
            tagFrame.size.width = labelFrame.size.width + buttonFrame.size.width - 15;
        ...
        if (lastView != nil) {
            tagFrame.origin.x = lastView.frame.origin.x + lastView.frame.size.width;
        }
    ...
    if (_mode == TLTagsControlModeEdit) {
        ...
        if (tagSubviews_.count == 0) {
            ...
            UIView *view = tagSubviews_.lastObject;
            frame.origin.x = view.frame.origin.x + view.frame.size.width;
        }
        tagInputField_.frame = frame;
...
- (void)deleteTagButton:(UIButton *)sender {
    ...
    NSInteger index = [tagSubviews_ indexOfObject:view];
    [tapDelegate tagsControl:self deleteTagIndex:index];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    [tapDelegate tagsControl:self sendMsg:textField];
    return YES;
}

- (void)textFieldChanged:(UITextField *)textField{
    [tapDelegate tagsControl:self inputFieldChange:textField];
}
